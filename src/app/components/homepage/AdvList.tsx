...
export default function AdvList() {
  ...
  useEffect(() => {
    ...
    advListRef.current?.addEventListener('scroll', onScrollEventListner)

    onScrollEventListner()
    windowResizeEventListner()
    return () => {
      window.removeEventListener('resize', windowResizeEventListner)
      advListRef.current?.removeEventListener('scroll', onScrollEventListner)
    }
  }, [advListRef, scrollByValue])
        ...
        <div className="flex space-x-5 w-fit py-10">
          {
            topAdvList.map((adv, index) => {
              return (
                // h-[212px]
                <div key={index} className="simple-border ml-2 shadow-lg rounded-2xl overflow-hidden bg-base-main ">
                  <div className="flex w-fit">
                    ...
                    <div className="p-2.5 sm:p-5 w-[200px] sm:w-[274px]">
                      <h4 className="text-lg sm:text-2xl font-semibold text-[#22285E] dark:text-gray-100">{adv.name}</h4>
                      <p className="text-sm font-medium sm:text-base text-[#A0A3BD] dark:text-slate-200">{adv.desc}</p>
